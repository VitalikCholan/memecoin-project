...
function getValue(value) {
  ...
  assert(bytes3.length <= WordSize, "value out-of-bounds", "BUFFER_OVERRUN", {
    buffer: bytes3,
    length: WordSize,
...
function pbkdf2(hash3, password, salt, opts) {
  const {
    ...
    PRF,
    PRFSalt
  } = pbkdf2Init(hash3, password, salt, opts);
  let prfW;
...
function validateDST(dst) {
  ...
  if (typeof dst === "string") return utf8ToBytes3(dst);
...
function expand_message_xmd(msg, DST, lenInBytes, H) {
  ...
  isBytes(DST);
  isNum(lenInBytes);
  if (DST.length > 255) DST = H(concatBytes2(utf8ToBytes3("H2C-OVERSIZE-DST-"), DST));
  const {
